...
export default function decorate(block) {
  ...
  block.closest('.block').classList.add('clean-list-small-title-container');
  ...
  if (lastRow) {
    ...
    if (cells.length === 3) {
      ...
      if (modifierType === 'modifier-row' && modifierName === 'grid-col-size') {
        block.style.setProperty('--clean-list-small-title-grid-cols', modifierValue);
        lastRow.remove();
      }
    }
  }
  ...
  rows.forEach((row, index) => {
    ...
    if (level === 1) {
      ...
      currentGroup.className = 'clean-list-small-title-group';
  ...
  block.querySelectorAll('.clean-list-small-title-group').forEach(group => {
    if (!group.children.length) {
      group.remove();
    }
  });

  // Add the clean-list-small-title class to the block
  block.classList.add('clean-list-small-title');
}